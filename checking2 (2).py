...
def save_bill_data(item_list, total_amount, customer_name, mobile_number, mode_of_payment):
    cursor = connection.cursor()
    ...
    for item in item_list:
        cursor.execute(sql, (str(uuid.uuid4()), item['name'], item['quantity'], item['price'], item['total'], customer_name, mobile_number, mode_of_payment))
    connection.commit()
    cursor.close()
...
def generate_bill():
    ...
    if st.button("Save Bill"):
        ...
        if not mobile_number.isdigit():
            ...
            return
        save_bill_data(selected_items, total_amount, customer_name, mobile_number, mode_of_payment)
